...
<Preview>
  <Story name="custom locale">
    {() => {
      const [state, setState] = useState("0.01");
      const setValue = ({ target }) => {
        setState(target.value.rawValue);
      };
      const [statePt, setStatePt] = useState("0.01");
      const setValuePt = ({ target }) => {
        setStatePt(target.value.rawValue);
      };
      return <>
        <Decimal label="Decimal - pt locale" value={statePt} onChange={setValuePt} locale="pt" delimiter=" " separator="," />
        <Decimal label="Decimal - en locale" value={state} onChange={setValue} locale="en"/>
      </>;
    }}
  </Story>
</Preview>
